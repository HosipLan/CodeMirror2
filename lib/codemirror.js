...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    function highlightLines(start, end) {
      if (mode.lookahead) {
        var n = start;
        start = Math.max(start - mode.lookahead, 0);
        if (start < n) changes.push({from: start, to: n});
      }
  ...
  Line.prototype = {
    ...
    highlight: function(mode, state, tabSize) {
      ...
      if (mode.lookahead) {
        for (var no = lineNo(this), i = no - mode.lookahead; i < no ;i++) {
          if (i >= 0) stream.previous.push(new StringStream(this.parent.lines[i].text, tabSize));
        }
      }
      var changed = false, curWord = st[0], prevWord;
      if (this.text == "" && mode.blankLine) mode.blankLine(state);
      while (!stream.eol()) {
        var style = mode.token(stream, state, this.lookahead(mode));
    ...
    lookahead: function (mode) {
      var lookahead = [];
      if (mode.lookahead) {
        var lines = this.parent.lookahead(lineNo(this), mode.lookahead);
        for (var i = 0; i < lines.length; i++) {
          lookahead.push(new StringStream(lines[i].text));
        }
      }
      return lookahead;
    },
  ...
  LeafChunk.prototype = {
    ...
    lookahead: function (no, num) {
      var lines = [];
      for (var i = no + num; i > no ;i--) {
        if (i >= 0 && this.parent.size > i) lines.push(this.lines[i]);
      }
      return lines;
    }
  };
