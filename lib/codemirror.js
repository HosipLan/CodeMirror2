...
var CodeMirror = (function() {
  ...
  Line.prototype = {
    ...
    highlight: function(mode, state, tabSize) {
      ...
      while (!stream.eol()) {
        var style = mode.token(stream, state, new LinesStream(this.parent, this));
  ...
  // for mode.token() method
  function LinesStream(lines, current) {
    this.lines = lines;
    this.current = current;
  }
  LinesStream.prototype = {
    isPrevious: function (tester) {
      var prev = this.lines.lineSiblings(this.current)[0];
      if (prev) {
        return tester(new StringStream(prev.text));
      }
      return false;
    },
    isNext: function (tester) {
      var next = this.lines.lineSiblings(this.current)[1];
      if (next) {
        return tester(new StringStream(next.text));
      }
      return false;
    }
  };
  ...
  LeafChunk.prototype = {
    ...
    lineSiblings: function (line) {
      var at = this.lines.indexOf(line);
      return [
        this.lines.slice(at-1, at).shift(),
        this.lines.slice(at+1, at+2).shift()
      ];
    }
  };
